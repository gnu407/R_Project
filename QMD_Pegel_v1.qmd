...
df_combined_01<- df_ursprung_01 %>%
  left_join(df_bock_01, by = "MESS_DATUM", suffix = c("_Ursprung", "_Bock"))

# CLEANUP - it got a bit dirty 
# Remove unwanted columns and rename Date_Ursprung to MESS_DATUM
df_combined_00 <- df_combined_01 %>%
  select(-MESS_DATUM,   # Remove the redundant MESS_DATUM from df_bock_01
         -Date_Bock) %>%      # Remove Date_Bock column
  rename(MESS_DATUM = Date_Ursprung)  # Rename Date_Ursprung to MESS_DATUM
...
p <- ggplot(df_combined_00, aes(x = Q_Bock, y = Q_Ursprung, color = factor(Year))) +
  ...
  ggtitle("Scatter Plot of Q_Bock vs. Q_Ursprung") +  # Add a title
  coord_fixed(ratio = 1)  # Ensure the x and y axes have the same scale
...
# Create the scatterplot with publication-quality formatting
p <- ggplot(df_combined_00, aes(x = Q_Bock, y = Q_Ursprung, color = factor(Year), text = paste("Year: ", Year))) +
  geom_point(alpha = 0.7, size = 2) +  # Scatterplot with semi-transparent points
  ...
  ggtitle("Scatter Plot of Q_Bock vs. Q_Ursprung")
